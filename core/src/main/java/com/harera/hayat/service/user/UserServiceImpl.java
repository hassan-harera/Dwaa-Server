...
class UserServiceImpl implements UserService {
    ...
    private long getUserId(String subject) {
        ...
        if (isPhoneNumber(subject)) {
            userId = userRepository.findByMobile(subject).get().getId();
        } else if (isEmail(subject)) {
            userId = userRepository.findByEmail(subject).get().getId();
        } else if (isUsername(subject)) {
            userId = userRepository.findByUsername(subject).get().getId();
        }
        return userId;
    }
